...
- **ノード塗り・ラベル色のバグ修正**（実装レビュー〔Opus〕で検出）: ノードの塗りとラベル色を SVG
...
- **エッジラベルの重なり調整**（実ブラウザ検証で検出）: ヘッドレス Chrome スクリーンショットによる
...
spec の検証方法 1〜4 は、実ブラウザ表示確認（検証方法 3 の確認内容の一部）を除き、実装ノート作成時に
再実行して確認した（npx 検出・frontmatter・レイアウトハーネス・node --check。行数確認も同時に実施）。
実ブラウザ表示確認は、実装時のヘッドレス Chrome スクリーンショット検証の実施分を引用する。
...
- `node --check`: テンプレート内の埋め込み `<script>` を現在のコミット内容から抽出し直して構文チェックし、
  OK を確認（620 行）
- 実ブラウザ確認（実装時に実施済みの結果を引用）: ヘッドレス Chrome スクリーンショット（1600×1000）で 4 サンプル
