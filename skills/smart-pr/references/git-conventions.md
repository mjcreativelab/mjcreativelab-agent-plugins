...
- `Closes #XX`: 対象 Issue を完全に解決する場合に使う（PR マージ時に自動で Issue をクローズする）
- `Refs #XX`: 参照のみ、または部分対応にとどまる場合に使う（自動クローズしない）
- 本文中で Issue に言及する説明文（背景・経緯等の地の文）では close / fix / resolve 系の語を Issue 番号に直接続けない（GitHub のキーワード一致は本文全体が対象のため、意図しない auto-close を招く）
