...
| 内容 | 振り分け先（簡易構成 / 標準構成） |
|------|-----------|
| 機能追加・修正・テスト等のコード変更 | `## 変更内容` / `## 実装内容`（末尾追記。標準構成で表・図があれば行・分岐の追記も検討） |
| 引数 `{プロンプト}` の指示内容 | `{プロンプト}` で指定されたセクション、未指定なら `## レビュアー向け補足` / `## 備考` |
| 設計判断・影響範囲・既知の制約 | `## レビュアー向け補足` / `## 設計判断（要点）`（追記） |
...
**本文**: [assets/pr-template.md](assets/pr-template.md) を使用。まず変更の性質から構成を選ぶ:
...
- プロジェクトに PULL_REQUEST_TEMPLATE（`.github/` 配下）がある場合はそのセクション構成（レビューポリシー等の独自セクション含む）を骨格にし、標準構成の図表を対応するセクションへ埋め込む

> **同期ノート**: [assets/pr-template.md](assets/pr-template.md) と上記の構成選択ルールは、`smart-issue-resolve` の「収束後のコミット・PR 作成 → PR タイトル・本文」へ複製している（スキル間ファイル参照ができないため意図的に二重化）。どちらかを変更したらもう一方も同期する。
