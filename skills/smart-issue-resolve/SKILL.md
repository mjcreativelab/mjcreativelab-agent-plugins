...
- リポジトリ内のコーディングルール集を Glob で探索する（`**/rules/*.md`・`**/guidelines/*.md`・`docs/` 配下の規約など）。Issue・変更ファイルのドメインに関連するものだけを Read する（例: `ddd-*` / `*architecture*` / `db-*` / `api-*` / `performance*` / `test-coverage*` / `security*`）
- 参照した実装計画（手順 2）の「依拠した前提」が指す基準

規約が見つからなければ下記の汎用観点のみで進める（クロスツール配布の degradation）。収集した基準は標準モードのテンプレート「プロジェクト固有基準」欄、および敵対的モードの Breaker 観点・Judge 照合対象に渡す。
...
1. **Breaker（Claude）** — 実装を「読む」のではなく「壊す」。反例・攻撃シナリオ・不変条件違反を列挙する。デフォルトは懐疑: 正常系（happy path）でしか成立しない実装は実在の弱点として扱い、善意・部分的な修正・「後続対応の見込み」に信用を与えない。セキュリティ観点（認可逸脱・インジェクション・秘密情報漏洩・TOCTOU・PII 露出）／仕様観点（受け入れ基準未充足・契約違反・後方互換性破壊・version skew／スキーマドリフト）／回帰観点／運用・保守・可用性観点（可観測性の欠落・デプロイ/ロールバックの脆さ・過度な結合やテスト容易性の低下・タイムアウト/リトライ欠如・障害時や依存劣化時の挙動・リソース枯渇・単一障害点）／データ整合性・性能観点（トランザクション境界・原子性・冪等性〔二重実行〕・並行更新の破れ・部分失敗・再入可能性・不可逆な状態変更・N+1 や過剰 I/O・計算量）／アーキテクチャ観点（レイヤー責務の逸脱・境界侵犯・未検証の実行パス）を横断する。「レビュー基準の収集」で得たプロジェクト固有ルールがあれば、その観点も攻撃シナリオに含める。可能なものは最小 failing テストとして書き、手順 6-5 のテストスコープで実行して検証する（pass したら仮説を破棄、fail したら検証済み反例として採用）。実行で確認できない仮説には `[UNVERIFIED]` を付す。反例テストは検証目的の使い捨てとして扱い、採用した欠陥の回帰テストとして残すもの以外（pass して破棄・不採用・未検証の仮説のテスト）は最終的な変更セットに含めない
