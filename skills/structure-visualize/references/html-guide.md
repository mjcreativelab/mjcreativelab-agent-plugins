...
| `nodes[].group` | - | 所属グループ ID。配色とエリア枠を駆動。無しは中立色・エリア外配置。**同一 group 内は `nodes` の配列順にそのまま縦 1 列で積まれる**（下記「エリア内の並び」） |
| `nodes[].members[]` | - | `{ name, type? }`。カラム・フィールド・属性の行。指定するとノードが可変高になる |
| `nodes[].detail` | - | `{ source?, description? }`。出典（ファイルパスや「会話」）と概要。クリックパネルに表示 |
| `edges[]` | - | `{ from, to, label?, cardinality? }`。from が to に依存する向き（flow・エリア間は from が左、エリア内は from が上）。label は関係種別（`implements` / `uses` 等） |
...
エリア内のノードは **`nodes` の配列順で上から下へ縦 1 列**に積まれる（エリア同士は横並び）。構成要素が増えても図が横に広がらないようにするための固定仕様で、エリア内の依存関係でノードの左右位置が決まることはない。

- **各 group のノードは、流れ（呼び出し順・依存の向き）どおりに並べて渡す**。並び順がそのまま図の上下になる
- 積み順で真下に来るエッジは直線、それ以外（1 つ以上跨ぐ・下から上へ戻る）は列の脇へ膨らむ曲線になる。跨ぎ・戻りが多いエリアは並び順を見直すと読みやすくなる
- エリア間のエッジと `flow` の配置は従来どおり左 → 右
