...
- **エリア内は縦積み**: エリア内のノードは入力順で縦 1 列に積み、エリア同士を横並びに配置する（構成要素が増えても図が横に広がらない）
- **依存フロー配置**: 依存の流れ・ER が主題の図では枠なしの階層配置
- **ノード詳細**: members（カラム・フィールド・属性）行、クリックで種別 / グループ / 出典 / 概要のパネル、hover ハイライト（パネル表示中は選択ノードのハイライトが保持される）、ズーム / パン
