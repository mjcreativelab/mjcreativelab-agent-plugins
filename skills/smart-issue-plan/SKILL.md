...
例: `/smart-issue-plan #42 --codex-review-loop` → Issue 番号: 42、レビューモード: standard
例: `/smart-issue-plan #42 -cdxarl` → Issue 番号: 42、レビューモード: adversarial

`{レビューモード}` が `off` でも、計画がセキュアな設計を要すると判定した場合は `adversarial` に自動昇格する（「Codex レビューループ」セクションのセキュリティ自動発動を参照）。`{レビューモード}` が `off` 以外になった場合の挙動は「Codex レビューループ」セクションを参照。
...
計画完成後、「Codex レビューループ」セクションの **レビューモードの確定（セキュリティ自動発動）** を評価する。`{レビューモード}` が `off` 以外に確定したらレビューループを実施し、その後は次のように分岐する:
...
> レビューループを実施し、かつ `{ループ明示}` = true の場合はこの手順をスキップする（オプション指定が投稿のオプトイン）。セキュリティ自動発動のみ（`{ループ明示}` = false）の場合、およびループのフォールバック（codex:rescue 利用不能）時は通常どおり実施する。
...
レビューループを実施した場合、投稿本文の末尾にレビュー済み表記（「Codex レビューループ」セクション参照）を付ける。
...
8b の再評価が終わった時点で「Codex レビューループ」セクションのレビューモードの確定を評価し、`{レビューモード}` が `off` 以外なら実施する。`{ループ明示}` = true で収束または打ち切り投稿を選んだ場合は 8c の承認ゲートをスキップして投稿し、レビュー済み表記を付ける（セキュリティ自動発動のみの場合は 8c の承認ゲートを維持する）。
...
1. **レビュー取得** — `{レビューモード}` に応じてレビューを取得する（下記「標準モード」「敵対的モード」）
...
5. **上限チェック** — 3 ラウンド終えても収束しない場合、残りの指摘の要約を提示して AskUserQuestion で確認する:
   ...
   - **打ち切って投稿** → 未収束のまま投稿フェーズへ（`{ループ明示}` = false のときは手順 6 の承認ゲートを通す。表記は「未収束で打ち切り」）
   - **中止** → 投稿せず、計画の現状をチャットに提示して終了

各ラウンドのモード・指摘数・採用数・不採用理由の要約を記録し、投稿後の完了報告に含める。
...
1. **Breaker（Claude）** — 計画を「破る」観点で、設計が耐えるべき具体的な攻撃シナリオ・脅威・欠落コントロールを列挙する。セキュリティ観点では STRIDE（なりすまし・改ざん・否認・情報漏洩・DoS・権限昇格）・信頼境界・認可漏れ・PII/秘密情報の露出・注入面を、一般観点では移行失敗・後方互換性破壊・品質ゲートの抜け・運用/保守/可用性の設計欠落（可観測性・デプロイ/ロールバック・障害時の劣化・単一障害点・過度な結合）を洗い出す。各シナリオに、計画のどの手順・前提が対処できていないかを付す
...
- 「Codex レビュー未実施」とユーザーに明示し、通常フロー（手順 6 / 8c の承認ゲートあり）に切り替える。セキュリティ自動発動で敵対的レビューを試みたケースでは「セキュリティ影響を検出したが Codex 不在のためレビュー未実施」と明示する
- レビュー済み表記は付けない
