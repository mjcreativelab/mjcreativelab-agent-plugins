...
2. Breaker が見落とした計画の欠陥（実現可能性・影響範囲の抜け・手順の妥当性・リスクの見落とし・運用/保守/可用性の設計欠落・データ整合性/性能〔トランザクション境界・冪等性・N+1〕・テストカバレッジ計画の不足・アーキテクチャ層責務の逸脱）があれば、独立レビューとして追加で挙げる（同じ 4 カテゴリで分類する。「5. プロジェクト固有基準」の提示があれば、それとの不整合も照合対象に含める）
...
- 根拠: <ファイルパス・行・確認方法>
- 分類理由: <なぜ真の欠陥か>
- 内容: <実装時に何が起きるか・なぜ計画が脆弱か・想定影響・計画をどう直すべきか>
